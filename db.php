...
dialog{position:absolute;background:lightyellow;border-radius:10px;border:1px solid;box-shadow:8px 8px 8px 0px grey;}
dialog p{display:grid;}dialog input{display:none;}cite{display:block;float:left;white-space:break-spaces;width:60px;}
</style></head><body onclick=dialog.style.display='none';>
<?php $conn=mysqli_connect($host,$user,$pass,$dbname);if(!$conn)log("connect failed");
if($_POST['del']){ foreach($_POST['del'] as $d){mysqli_query($conn,$del."'".$d."'");}
...
<tr id=th><th><input onclick="form.dels.forEach(v=>v.checked=!v.checked)" type=checkbox></th>';foreach($cols as $v){echo"<th>".strtoupper($v)."</th>";}echo"</tr>";
while($row=mysqli_fetch_array($page,MYSQLI_NUM)){
  echo"<tr ondblclick=dialog.style.display='block';dialog.style.top=event.clientY+'px';dialog.style.left=event.clientX+'px';dialog.innerHTML='<cite&gt;'+th.innerText.trim()+'</cite&gt;<p&gt;'+this.innerHTML+'</p&gt;'><td><input id=dels name=del[] value=".$row[0]." type=checkbox>";
  foreach($row as $v){echo"</td><td>".(strlen($v)<$colsize?"<span>$v</span>":"<span onclick='this.innerText=this.attributes.data.value' data='$v'>".substr($v,0,$colsize)."...</span>");}
  echo"</td><tr>";
}echo"</table></form>";
echo"<dialog id=dialog draggable=true onclick=event.stopPropagation();></dialog>";
...
for($i=$left;$i<=$right;$i++){if($i==$pg+1){echo" <b><a href=\"$url&pg=".($i-1)."\">$i</a></b> ";}else{echo" <a href=\"$url&pg=".($i-1)."\">$i</a> ";}}
