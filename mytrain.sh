...
ok() {
  mpg123 ~/Musics/Queen\ -\ Love\ Of\ My\ Life.mp3
}
...
query() {
  ...
  _to=$3
  _num=$4
  res=$(curl -k -A "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/46.0.2490.71 Safari/537.36" \
  "https://kyfw.12306.cn/otn/leftTicket/queryA?leftTicketDTO.train_date=$_date\
  &leftTicketDTO.from_station=$_from\
  &leftTicketDTO.to_station=$_to\
  &purpose_codes=ADULT" 2>/dev/null | jq ".data[$_num]\
  |{ NO_:.queryLeftNewDTO.station_train_code,\
  from:.queryLeftNewDTO.from_station_name,\
  to:.queryLeftNewDTO.to_station_name,\
  yw:.queryLeftNewDTO.yw_num,\
  rw:.queryLeftNewDTO.rw_num,\
